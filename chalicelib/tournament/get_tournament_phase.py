...
from playerstars_adapters import PlayerTournamentAdapter, TeamTournamentAdapter, DuelAdapter


def get_player_tournament_adapter():
    return PlayerTournamentAdapter(Settings.PLAYER_TOURNAMENT_TABLE_NAME, Settings.DYNAMODB_URL)
...
def get_tournament_phase(entity_id):
    try:
        ...
        request = GetTournamentPhaseRequestModel(tournament_id=entity_id, player_id=player_id)
        ...
        interactor = GetTournamentPhaseInteractor(request=request, adapters=adapters)
        response = interactor.run()
        if response:
            return success(response())
        return not_found(f"Tournament {entity_id} not found for the player {player_id}")
