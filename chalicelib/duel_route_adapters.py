from playerstars_adapters import PreDuelAdapter, DuelAdapter as DuelAdapterDynamo, ConsoleAdapter, \
    NotificationAdapter as NotificationAdapterDynamo, PlayerAdapter, TeamAdapter, ValuesAdapter
...
def get_schedule_task_adapter():
    schedule_task_adapter = AwsScheduleTaskAdapter(
        name='duel-finish',
        task_identifier=Settings.DUEL_SCHEDULED_FINISHER_NAME, aws_region=Settings.AWS_DEFAULT_REGION)
    return schedule_task_adapter


def get_era_adapter():
    era_adapter = EventReminderAssistantAdapter(table_name=Settings.ERA_TABLE_NAME)
    return era_adapter


def get_aws_task_scheduler_adapter():
    aws_task_scheduler_adapter = AwsTaskSchedulerAdapter(
        name='duel-finisher', lambda_runner=f'{Settings.ERA_AWS_LAMBDA_FUNCTION_NAME}-{Settings.ERA_RUNNER_NAME}')
    return aws_task_scheduler_adapter


def get_values_adapter():
    values_adapter = ValuesAdapter(table_name=Settings.VALUES_TABLE_NAME)
